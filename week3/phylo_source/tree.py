...
# Add hashable set extension for Codon
@extend
class set:
    def __hash__(self):
        MAX = int.MAX
...
class Tree:
    ...
    def __init__(self, root):
        ...
        for _ in range(leaf_count):
            self._leaves.append(leaves_unsorted[0])  # Initialize with placeholder
...
class TreeNode:
    ...
    def __init__(self, children=None, distances=None, index=None):
        ...
        if index is None:
            ...
            self._index = -1
            self._children = [i for i in children]  # Convert to list
    ...
    def get_leaves(self):
        ...
        # delegate to function to reduce overhead of recursive function calling
    ...
    def to_newick(self, labels=None, include_distance=True, round_distance=None):
        ...
        if self.is_leaf():
            if labels is not None:
                ...
                # Characters that are part of the Newick syntax are illegal
                illegal_chars = [",",":",";","(",")"]
                for char in illegal_chars:
                    if char in label:
                        raise ValueError(f"Label '{label}' contains illegal character '{char}'")
            else:
            ...
            child_strings = [child.to_newick(labels, include_distance, round_distance) for child in self._children]
            if include_distance:
                if round_distance is None:
...
def as_binary(tree_or_node):
    ...
    if isinstance(tree_or_node, Tree):
        ...
        raise TypeError(f"Expected 'Tree' or 'TreeNode', not {type(tree_or_node).__name__}")
...
def _as_binary(node: TreeNode):
    ...
    if len(children) == 0:
        ...
        binary_results = [_as_binary(child) for child in children]
        rem_children = [result[0] for result in binary_results]
        distances = [result[1] for result in binary_results]
        ...
        while len(rem_children) > 0:
            if current_div_node is None:
                ...
                current_div_node = TreeNode(rem_children[:2], distances[:2])
                ...
                current_div_node = TreeNode([current_div_node, rem_children[0]], [0.0, distances[0]])
        ...
        # -> Keep node unchanged
        binary_results = [_as_binary(child) for child in children]
        binary_children = [result[0] for result in binary_results]
        distances = [result[1] for result in binary_results]
